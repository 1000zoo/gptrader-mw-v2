...
def _markdown_bytes(replay: object, decomposition: object | None) -> bytes:
    ...
    answers = [
        ("1. 기존 두 실패 값이 재현됐는가?", _answer_reproduced(replay)),
        (
            "2. 2.3526은 어느 군집 pair에서 발생했는가?",
            "재현 실패로 pair 해부를 중단했다."
            if top_pair is None
            else (
        ...
        (
            "3. 어떤 피처가 거리의 대부분을 만들었는가?",
            _top_features_answer(decomposition),
        ),
        (
            "4. 해당 이동은 mean에만 존재하는가, median/medoid에서도 유지되는가?",
            "location distance rows를 함께 확인한다.",
        ),
        (
            "5. 소수 극단값을 제외하면 거리가 얼마나 줄어드는가?",
            "exclusion sensitivity rows를 함께 확인한다.",
        ),
        (
            "6. OOD 4.631%는 어느 군집과 피처에서 주로 발생하는가?",
            _ood_answer(decomposition),
        ),
        (
            "7. centroid drift와 OOD 실패는 같은 군집·같은 피처에서 발생하는가?",
            "cluster diagnostics와 OOD sample contribution을 대조한다.",
        ),
        (
            "8. covariance를 반영한 component distance에서도 동일한 이상이 확인되는가?",
            "distance comparison CSV의 Mahalanobis/KL/Bhattacharyya/Wasserstein 계열을 본다.",
        ),
        (
            "9. 3일/7일 subsample에서도 결론이 유지되는가?",
            "offset subsample rows를 함께 확인한다.",
        ),
        (
            "10. 실패 원인은 무엇으로 분류되는가?",
            _cause_answer(decomposition),
        ),
    ]
    lines = [
...
def _answer_reproduced(replay: object) -> str:
    if replay.status.status != "reproduced":
        return f"아니오. `{replay.status.mismatch_classification}` 단계에서 중단했다."
    return "예. centroid distance와 OOD exceedance rate가 허용 오차 내에서 재현됐다."
...
def _top_features_answer(decomposition: object | None) -> str:
    if decomposition is None or not decomposition.feature_contributions:
        return "재현 실패로 feature contribution을 산출하지 않았다."
    rows = sorted(
...
def _ood_answer(decomposition: object | None) -> str:
    if decomposition is None or not decomposition.ood_by_component:
        return "재현 실패로 OOD component 해부를 산출하지 않았다."
...
def _cause_answer(decomposition: object | None) -> str:
    ...
    return ", ".join(causes) if causes else "결정적 원인 신호 없음."
