...
def _completion_fixture() -> SimpleNamespace:
    ...
    full_ood = (SimpleNamespace(sample_scope="full_sample", spacing_days=None, offset=None, primary_component_index=0, primary_component_fingerprint=fingerprint, numerator=24, denominator=409, rate=24 / 409, distance_source="existing_primary_ood_row", threshold_source="frozen_component_chi_square_threshold"),)
    offset_ood = tuple(SimpleNamespace(sample_scope="offset_subsample", spacing_days=row.spacing_days, offset=row.offset, primary_component_index=0, primary_component_fingerprint=fingerprint, numerator=1, denominator=10, rate=.1, distance_source="existing_primary_ood_row", threshold_source="frozen_component_chi_square_threshold") for row in offsets)
    receipts = tuple(SimpleNamespace(global_index=index, anchor_at=f"anchor-{index:04d}", half_label="A" if index < 820 else "B", half_component_index=1, half_component_fingerprint=half_fingerprint, matched_primary_component_index=3, matched_primary_component_fingerprint=fingerprint, primary_component_index=0, primary_component_fingerprint=fingerprint, squared_mahalanobis=1.0, ood_threshold=2.0, ood_exceeds=False, assignment_source="existing_reproduced_assignments") for index in range(1641))
