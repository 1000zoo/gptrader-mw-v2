...
async def test_regime_uptrend():
    service = RegimeService()
    k1h = _build_klines(np.linspace(100, 200, 220))
    k15m = _build_klines(np.linspace(100, 210, 420))
    result = await service.classify_regime(
        "BTCUSDT", k1h, k15m, ind_params=IndParams(), computed_at=datetime.now(timezone.utc)
    )
    assert result.regime == UPTREND


async def test_regime_downtrend():
    service = RegimeService()
...
async def test_regime_transition_by_spike():
    service = RegimeService()
...
async def test_regime_unknown_when_data_short():
    service = RegimeService()
